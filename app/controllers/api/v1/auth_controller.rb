class Api::V1::AuthController < ApplicationController
  ...
  def create
    ...
    if @coach && @coach.authenticate(user_login_params[:password])
      ...
      render json: {type: "coach", coach: CoachSerializer.new(@coach), jwt: token}, status: :accepted
    elsif @player && @player.authenticate(user_login_params[:password])
      token = encode_token({player_id: @player.id})
      render json: {type: "player", player: PlayerSerializer.new(@player), jwt: token}, status: :accepted
    else
